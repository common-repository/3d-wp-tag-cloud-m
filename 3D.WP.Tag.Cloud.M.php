...
Plugin URI: 
Version: 3.2.7
Description: A Multiple Clouds variant of 3D WP Tag Cloud: Creates multiple instances widget that draws and animates HTML5 canvas based tag clouds. Supports Shortcodes. Rotates up to 9 types of content in a widget, controlled by a menu. Supports 90 shapes. Uses Graham Breach's JS class TagCanvas v. 2.8 and includes most of its 80+ options in the Control Panel.
Author: Peter Petrov
Author URI: 
License: LGPL v3
	...
	class wpTagCanvasWidgetM extends WP_Widget {
		function wpTagCanvasWidgetM () {
			...
			'wpTagCanvasWidgetM', // Base ID
				...
				array( 'description' => __( 'Draws and animates multiple 3D tag cloud. Generates a Shortcode for each instance.', 'text_domain' ), ) // Args
			);
			// register Shortcode
...
<!-- Building tag containers -->
			<?php
				...
				if( $auth_menu_item == 'on'){
					...
					foreach( $users as $user ){ 
						...
						echo '<a href="'.$userPostsURL.'" target="'.$auth_target.'">'.$userAvatar, $userFName.'<br>'.$userLName.'<br>('.$userPosts.')</a>';
					};	
					echo '</div>';
				}
				if( $cat_menu_item == 'on'){
				...
				if( $men_menu_item == 'on'){
					...
					$menu_args = array ('menu' => $all_menu_name, 'container' => false, 'items_wrap' => '%3$s'); 
			...
			?>	
		<div id='all_tag_canvas_wrap_<?= $inst_id; ?>' style="margin-left: auto; margin-right: auto;">
			<canvas title="" id="all_tag_canvas_<?= $inst_id; ?>" style="margin-left: auto; margin-right: auto;"></canvas>
		</div>
		<br>
		
<!-- Preparing menu container -->
		<div id='all_in_one_menu_container_wrap_<?= $inst_id; ?>' style="margin-left: auto; margin-right: auto;">
			<div id="all_in_one_menu_container_<?= $inst_id; ?>"  style="height: <?= $all_menu_height;?>px; text-align: center; display: none;">
			<?php
				if( $arch_menu_item == 'on'){
		...
		<div id='all_menu_canvas_wrap_<?= $inst_id; ?>' style="margin-left: auto; margin-right: auto;">
			<canvas title="<?php if($all_m_tooltip == 'on' && $all_menu_type == 'dynamic'){echo 'Drag or Click';}; ?>" id="all_menu_canvas_<?= $inst_id; ?>" height="<?= $all_menu_height; ?>" style="margin-left: auto; margin-right: auto; display: none;"></canvas>
		</div>
		<script type="text/javascript">
...
// Archives options update
		...
		$tag_option['arch_initial_z'] = $new_instance['arch_initial_z'];
		...
		$tag_option['arch_outline_dash'] = $new_instance['arch_outline_dash'];
		$tag_option['arch_outline_dash_space'] = $new_instance['arch_outline_dash_space'];
		$tag_option['arch_outline_dash_speed'] = $new_instance['arch_outline_dash_speed'];
		...
		$tag_option['arch_target'] = $new_instance['arch_target'];
...
// Authors options update
		...
		$tag_option['auth_initial_z'] = $new_instance['auth_initial_z'];
		...
		$tag_option['auth_outline_dash'] = $new_instance['auth_outline_dash'];
		$tag_option['auth_outline_dash_space'] = $new_instance['auth_outline_dash_space'];
		$tag_option['auth_outline_dash_speed'] = $new_instance['auth_outline_dash_speed'];
...
// Categories options update
		...
		$tag_option['cat_initial_z'] = $new_instance['cat_initial_z'];
		...
		$tag_option['cat_numberop'] = $new_instance['cat_numberop'];
		$tag_option['cat_outline_color'] = $new_instance['cat_outline_color'];
		$tag_option['cat_outline_dash'] = $new_instance['cat_outline_dash'];
		$tag_option['cat_outline_dash_space'] = $new_instance['cat_outline_dash_space'];
		$tag_option['cat_outline_dash_speed'] = $new_instance['cat_outline_dash_speed'];
		...
		$tag_option['cat_target'] = $new_instance['cat_target'];
...
// Links options update
		...
		$tag_option['lin_initial_z'] = $new_instance['lin_initial_z'];
		...
		$tag_option['lin_outline_dash'] = $new_instance['lin_outline_dash'];
		$tag_option['lin_outline_dash_space'] = $new_instance['lin_outline_dash_space'];
		$tag_option['lin_outline_dash_speed'] = $new_instance['lin_outline_dash_speed'];
...
// Menu options update
		...
		$tag_option['men_initial_z'] = $new_instance['men_initial_z'];
		...
		$tag_option['men_outline_dash'] = $new_instance['men_outline_dash'];
		$tag_option['men_outline_dash_space'] = $new_instance['men_outline_dash_space'];
		$tag_option['men_outline_dash_speed'] = $new_instance['men_outline_dash_speed'];
		...
		$tag_option['men_target'] = $new_instance['men_target'];
...
// Pages options update
		...
		$tag_option['pag_initial_z'] = $new_instance['pag_initial_z'];
		...
		$tag_option['pag_outline_dash'] = $new_instance['pag_outline_dash'];
		$tag_option['pag_outline_dash_space'] = $new_instance['pag_outline_dash_space'];
		$tag_option['pag_outline_dash_speed'] = $new_instance['pag_outline_dash_speed'];
...
// Post Tags options update
		...
		$tag_option['pos_initial_z'] = $new_instance['pos_initial_z'];
		...
		$tag_option['pos_numberot'] = $new_instance['pos_numberot'];
		$tag_option['pos_outline_color'] = $new_instance['pos_outline_color'];
		$tag_option['pos_outline_dash'] = $new_instance['pos_outline_dash'];
		$tag_option['pos_outline_dash_space'] = $new_instance['pos_outline_dash_space'];
		$tag_option['pos_outline_dash_speed'] = $new_instance['pos_outline_dash_speed'];
		...
		$tag_option['pos_target'] = $new_instance['pos_target'];
...
// Recent Posts options update
		...
		$tag_option['rec_initial_z'] = $new_instance['rec_initial_z'];
		...
		$tag_option['rec_outline_dash'] = $new_instance['rec_outline_dash'];
		$tag_option['rec_outline_dash_space'] = $new_instance['rec_outline_dash_space'];
		$tag_option['rec_outline_dash_speed'] = $new_instance['rec_outline_dash_speed'];
...
// Page/Post Links options update
		...
		$tag_option['ppl_outline_dash'] = $new_instance['ppl_outline_dash'];
		$tag_option['ppl_outline_dash_space'] = $new_instance['ppl_outline_dash_space'];
		$tag_option['ppl_outline_dash_speed'] = $new_instance['ppl_outline_dash_speed'];
		...
		$tag_option['ppl_target'] = $new_instance['ppl_target'];
		...
		delete_option("3dwptagcloud-m-widget-instance");
		add_option("3dwptagcloud-m-widget-instance", $this->id);
...
function my_m_admin_notice() {
	...
	$message = "<b>" . $plugin_m_data['Name'] . "</b>:<br>Dear users, this is the last version of the plugin. Starting from 17 Jan. 2018 it will no longer be supported. I'd like to thank you all who trusted and used it. Good luck!";
	$display_info = false;
