...
<script type="text/javascript">
	jQuery(window).load(function() {
		...
		if(check != ''){
			...
			jQuery(function() {
				jQuery( "#accordion-2, #accordion-4, #accordion-5" ).accordion({heightStyle: "content", collapsible: true, active: false});
				jQuery( "#accordion-2, #accordion-4, #accordion-5" ).accordion({beforeActivate: function( click ) {if(jQuery("div.ui-tooltip-content")[0]){return false};}});				
			});
		};
		jQuery( "#wihead, #accordion-2" ).keydown(function( event ) {
		  if ( event.which == 13 ) {
		   event.preventDefault();
		  }
		});
		jQuery("#savewidget").on("submit", function() { if(jQuery("div.ui-tooltip-content")[0]){return false};});
		for(var i=0; i<jQuery("#<?=$this->get_field_id('all_font_cf'); ?> option").length; i++){
			WebFont.load({google: {families: [jQuery("#<?=$this->get_field_id('all_font_cf'); ?> option").eq(i).attr("value")]}});
			jQuery("#<?=$this->get_field_id('all_font_cf'); ?> option").eq(i).css('font-family',jQuery("#<?=$this->get_field_id('all_font_cf'); ?> option").eq(i).attr("value"));
		};	
	});
...
// "Not less than two menu items" check and "Unchecked starting item" control
	function procedure_1(e){
		var on = 0;
		jQuery('.check-box').each(function() {if(jQuery( this ).prop('checked')==true){on++;}});
		if(on==1&&e.checked==false){
...
<?php if( $check_widget_1 == "" && $check_widget_2!= "" ){
			echo '<style>div.widget-control-actions div.alignright input.widget-control-save, div.widget-control-actions div.alignleft a.widget-control-close {display: none;}</style><p id="warning" style="margin: 10px 5px 5px; font-size: 14px; text-align: justify;">This widget can be customized only through <a href="../wp-admin/widgets.php"><b>WP Admin panel > Appearance > Widgets</b></b> page, where Accessibility mode in <b>Screen Options</b> (top right corner of the page) has to be enabled.</p>'; 
		} 
		else {if( $check_widget_1 == ""){
				echo '<style>div.widget-control-actions div.alignright input.widget-control-save, div.widget-control-actions div.alignleft a.widget-control-close {display: none;}</style><p id="warning" style="margin: 10px 5px 5px; font-size: 14px; text-align: justify;">In order to edit this widget instance and get a Shortcode you have to enable accessibility mode in <b>Screen Options</b> (right top corner of this page).</p>'; 
			}
			else    {
				$mshortcode_id = preg_replace('!^[^\d]+!uis','',$this->id);
			};
		}
?>
<!-- CP Template -->
<div id="wihead" style="visibility: hidden;">
	<div style="float: left;">
		...
		<div style="font-weight: normal;">
			<?php 
				$murl_check = $_SERVER["REQUEST_URI"];
				if(	strpos($murl_check, 'wp_inactive_widgets') == true||strpos($murl_check, 'sidebar') == false){
					if (isset($this->id) && !empty($mshortcode_id)) {
					echo 'Shortcode: <input value="[tc-m id=&#34;'.$mshortcode_id.'&#34;]" style="float: right; line-height: 12px; height: 14px; border-radius: 4px; font-size: 12px; padding: 2px 4px; border: 0; width: 68px;" onclick="select()"  title="Put your cloud in a page/post via this Shortcode.">';
					}
	...
	<div id="toti" onmouseout="jQuery(this).css({'color':'#dc143c'});" onmouseover="jQuery(this).css({'color':'#b01030'});">
		...
		<?php if( $tooltip_status == "on" ){ echo ' checked="checked"'; } ?> onclick="jQuery('#accordion-2, #wihead').tooltip({content: function() {var element = jQuery( this ); var html_text=element.attr('title'); return html_text;}, position: {  my: 'left top+20',  at: 'left bottom'}}); ">on
	...
	<label style="display: inline-block; margin: 0 0 0 4px" title="Widget's width" for="<?=$this->get_field_id('width'); ?>">
...
<div id="accordion-2" style="background: #fff; padding: 2px 0 1px; visibility: hidden; " <?php if( $check_widget_1 == "" ){ echo ' hidden'; } ?>>
	...
	<div class="section_content">
		...
		<div style="">
			...
			style="float: left; width: 157px; margin: 0 0 10px 0;" for="<?=$this->get_field_id('all_menu_font'); ?>">
				Web Safe Font
				<select id="<?=$this->get_field_id('all_menu_font'); ?>" name="<?=$this->get_field_name('all_menu_font'); ?>">
					<option style="background: #f1f1f1; font-family: Arial;" value="Arial" <?php if( $all_menu_font == "Arial" ){ echo ' selected'; } ?>>Arial</option>
					<option style="background: #f1f1f1; font-family: Arial Black;" value="Arial Black" <?php if( $all_menu_font == "Arial Black" ){ echo ' selected'; } ?>>Arial Black</option>
					<option style="background: #f1f1f1; font-family: Arial Narrow;" value="Arial Narrow" <?php if( $all_menu_font == "Arial Narrow" ){ echo ' selected'; } ?>>Arial Narrow</option>
					<option style="background: #f1f1f1; font-family: Avant Garde;" value="Avant Garde" <?php if( $all_menu_font == "Avant Garde" ){ echo ' selected'; } ?>>Avant Garde</option>
					<option style="background: #f1f1f1; font-family: Calibri;" value="Calibri" <?php if( $all_menu_font == "Calibri" ){ echo ' selected'; } ?>>Calibri</option>
					<option style="background: #f1f1f1; font-family: Candara;" value="Candara" <?php if( $all_menu_font == "Candara" ){ echo ' selected'; } ?>>Candara</option>
					<option style="background: #f1f1f1; font-family: Century Gothic;" value="Century Gothic" <?php if( $all_menu_font == "Century Gothic" ){ echo ' selected'; } ?>>Century Gothic</option>
					<option style="background: #f1f1f1; font-family: Comic Sans MS;" value="Comic Sans MS" <?php if( $all_menu_font == "Comic Sans MS" ){ echo ' selected'; } ?>>Comic Sans MS</option>
					<option style="background: #f1f1f1; font-family: Franklin Gothic Medium;" value="Franklin Gothic Medium" <?php if( $all_menu_font == "Franklin Gothic Medium" ){ echo ' selected'; } ?>>Franklin Gothic Medium</option>
					<option style="background: #f1f1f1; font-family: Futura;" value="Futura" <?php if( $all_menu_font == "Futura" ){ echo ' selected'; } ?>>Futura</option>
					<option style="background: #f1f1f1; font-family: Geneva;" value="Geneva" <?php if( $all_menu_font == "Geneva" ){ echo ' selected'; } ?>>Geneva</option>
					<option style="background: #f1f1f1; font-family: Gill Sans;" value="Gill Sans" <?php if( $all_menu_font == "Gill Sans" ){ echo ' selected'; } ?>>Gill Sans</option>
					<option style="background: #f1f1f1; font-family: Helvetica;" value="Helvetica" <?php if( $all_menu_font == "Helvetica" ){ echo ' selected'; } ?>>Helvetica</option>
					<option style="background: #f1f1f1; font-family: Impact;" value="Impact" <?php if( $all_menu_font == "Impact" ){ echo ' selected'; } ?>>Impact</option>
					<option style="background: #f1f1f1; font-family: Lucida Grande;" value="Lucida Grande" <?php if( $all_menu_font == "Lucida Grande" ){ echo ' selected'; } ?>>Lucida Grande</option>
					<option style="background: #f1f1f1; font-family: Lucida Sans Unicode;" value="Lucida Sans Unicode" <?php if( $all_menu_font == "Lucida Sans Unicode" ){ echo ' selected'; } ?>>Lucida Sans Unicode</option>
					<option style="background: #f1f1f1; font-family: Optima;" value="Optima" <?php if( $all_menu_font == "Optima" ){ echo ' selected'; } ?>>Optima</option>
					<option style="background: #f1f1f1; font-family: Segoe UI;" value="Segoe UI" <?php if( $all_menu_font == "Segoe UI" ){ echo ' selected'; } ?>>Segoe UI</option>
					<option style="background: #f1f1f1; font-family: Tahoma;" value="Tahoma" <?php if( $all_menu_font == "Tahoma" ){ echo ' selected'; } ?>>Tahoma</option>
					<option style="background: #f1f1f1 font-family: Trebuchet MS;" value="Trebuchet MS" <?php if( $all_menu_font == "Trebuchet MS" ){ echo ' selected'; } ?>>Trebuchet MS</option>
					<option style="background: #f1f1f1 font-family: Verdana;" value="Verdana" <?php if( $all_menu_font == "Verdana" ){ echo ' selected'; } ?>>Verdana</option>
					<option style="font-family: Baskerville;" value="Baskerville" <?php if( $all_menu_font == "Baskerville" ){ echo ' selected'; } ?>>Baskerville</option>
					<option style="font-family: Big Caslon;" value="Big Caslon" <?php if( $all_menu_font == "Big Caslon" ){ echo ' selected'; } ?>>Big Caslon</option>
					<option style="font-family: Bodoni MT;" value="Bodoni MT" <?php if( $all_menu_font == "Bodoni MT" ){ echo ' selected'; } ?>>Bodoni MT</option>
					<option style="font-family: Book Antiqua;" value="Book Antiqua" <?php if( $all_menu_font == "Book Antiqua" ){ echo ' selected'; } ?>>Book Antiqua</option>
					<option style="font-family: Calisto MT;" value="Calisto MT" <?php if( $all_menu_font == "Calisto MT" ){ echo ' selected'; } ?>>Calisto MT</option>
					<option style="font-family: Cambria;" value="Cambria" <?php if( $all_menu_font == "Cambria" ){ echo ' selected'; } ?>>Cambria</option>
					<option style="font-family: Didot;" value="Didot" <?php if( $all_menu_font == "Didot" ){ echo ' selected'; } ?>>Didot</option>
					<option style="font-family: Garamond;" value="Garamond" <?php if( $all_menu_font == "Garamond" ){ echo ' selected'; } ?>>Garamond</option>
					<option style="font-family: Georgia;" value="Georgia" <?php if( $all_menu_font == "Georgia" ){ echo ' selected'; } ?>>Georgia</option>
					<option style="font-family: Goudy Old Style;" value="Goudy Old Style" <?php if( $all_menu_font == "Goudy Old Style" ){ echo ' selected'; } ?>>Goudy Old Style</option>
					<option style="font-family: Hoefler Text;" value="Hoefler Text" <?php if( $all_menu_font == "Hoefler Text" ){ echo ' selected'; } ?>>Hoefler Text</option>
					<option style="font-family: Lucida Bright;" value="Lucida Bright" <?php if( $all_menu_font == "Lucida Bright" ){ echo ' selected'; } ?>>Lucida Bright</option>
					<option style="font-family: Palatino;" value="Palatino" <?php if( $all_menu_font == "Palatino" ){ echo ' selected'; } ?>>Palatino</option>
					<option style="font-family: Palatino Linotype;" value="Palatino Linotype" <?php if( $all_menu_font == "Palatino Linotype" ){ echo ' selected'; } ?>>Palatino Linotype</option>
					<option style="font-family: Perpetua;" value="Perpetua" <?php if( $all_menu_font == "Perpetua" ){ echo ' selected'; } ?>>Perpetua</option>
					<option style="font-family: Rockwell;" value="Rockwell" <?php if( $all_menu_font == "Rockwell" ){ echo ' selected'; } ?>>Rockwell</option>
					<option style="font-family: Rockwell Extra Bold;" value="Rockwell Extra Bold" <?php if( $all_menu_font == "Rockwell Extra Bold" ){ echo ' selected'; } ?>>Rockwell Extra Bold</option>
					<option style="font-family: Times New Roman;" value="Times New Roman" <?php if( $all_menu_font == "Times New Roman" ){ echo ' selected'; } ?>>Times New Roman</option>
					<option style="background: #f1f1f1; font-family: Andale Mono;" value="Andale Mono" <?php if( $all_menu_font == "Andale Mono" ){ echo ' selected'; } ?>>Andale Mono</option>
					<option style="background: #f1f1f1; font-family: Consolas;" value="Consolas" <?php if( $all_menu_font == "Consolas" ){ echo ' selected'; } ?>>Consolas</option>
					<option style="background: #f1f1f1; font-family: Courier New;" value="Courier New" <?php if( $all_menu_font == "Courier New" ){ echo ' selected'; } ?>>Courier New</option>
					<option style="background: #f1f1f1; font-family: Lucida Console;" value="Lucida Console" <?php if( $all_menu_font == "Lucida Console" ){ echo ' selected'; } ?>>Lucida Console</option>
					<option style="background: #f1f1f1; font-family: Lucida Sans Typewriter;" value="Lucida Sans Typewriter" <?php if( $all_menu_font == "Lucida Sans Typewriter" ){ echo ' selected'; } ?>>Lucida Sans Typewriter</option>
					<option style="background: #f1f1f1; font-family: Monaco;" value="Monaco" <?php if( $all_menu_font == "Monaco" ){ echo ' selected'; } ?>>Monaco</option>
					<option style="font-family: Copperplate;" value="Copperplate" <?php if( $all_menu_font == "Copperplate" ){ echo ' selected'; } ?>>Copperplate</option>
					<option style="font-family: Papyrus;" value="Papyrus" <?php if( $all_menu_font == "Papyrus" ){ echo ' selected'; } ?>>Papyrus</option>
					<option style="background: #f1f1f1; font-family: Brush Script MT;" value="Brush Script MT" <?php if( $all_menu_font == "Brush Script MT" ){ echo ' selected'; } ?>>Brush Script MT</option>
				</select>
			</label>
		</div>
		<div style="">
			<div style="display: inline-block;">
				...
				<label style="height: 46px; float: right; margin: 0 16px 5px 0;" title="Color of the tag text: Empty to use the default color." for="<?=$this->get_field_id('all_m_fontcolor'); ?>">
					Font Color
					<br>
			...
			<div style="display: inline-block;">
				...
				<label style="height: 46px; float: right; margin: 0 0 5px;" title="Color of Menu tags' border: Empty for the same as the text color." for="<?=$this->get_field_id('all_m_bordercolor'); ?>">
					Border Color
					<br>
			...
			<div style="display: inline-block;">
				...
				<label style="height: 46px; float: right;" title="Colour of the shadow behind each tag: Empty for no shadow." for="<?=$this->get_field_id('all_m_shadow'); ?>">
					Shadow Color
					<br>
					<span class="hash">#</span>
					<div class="siwraper">
			...
			<label style="padding: 0 0 0 60px; width: 100%; margin: 0 0 5px;" title="Change this label if you are going to show accordion, portfolio or slider type of content under <span class='green'>menu</span> option. For instructions on how to put accordion, portfolio or slider types of content in the cloud go to <span class='green'>Cloud Content Tips</span>, item <span class='green'>5</span>, under <span style='font-weight: bold; color: #dc143c'>GUIDE & TIPS</span> section." for="<?=$this->get_field_id('menu_label'); ?>">
				*
				<select id="<?=$this->get_field_id('menu_label'); ?>" name="<?=$this->get_field_name('menu_label'); ?>">
					<option value="Accordions" <?php if( $menu_label == "Accordions" ){ echo ' selected'; } ?>>Accordions</option>
					<option value="Menu" <?php if( $menu_label == "Menu" ){ echo ' selected'; } ?>>Menu</option>
			...
			<div class="thin-spacer"></div>		
			<label style="width: 83px;" title="When <span class='green'>Menu Type</span> is set to <span class='green'>dynamic</span>: Initial radius of menu ring." for="<?=$this->get_field_id('all_m_radius_x'); ?>">
				Radius 
				<br>
			...
			<div style="display: inline-block;">
				<label style="width: 83px; height: 46px;" title="Background color of tags: Empty for no background." for="<?=$this->get_field_id('all_m_bgcolor'); ?>">
					Background
					<br>
					<span class="hash">#</span>
					<div class="siwraper">
				...
				<label style="width: 83px; height: 46px; float: left;" title="Background color of the Active Menu tag: Empty for no active background." for="<?=$this->get_field_id('active_bg_color'); ?>">
					Active BG
					<br>
					<span class="hash">#</span>
					<div class="siwraper">
				...
				<label style="height: 46px; float: left;" title="Color of the active tag highlight: Empty for no outline." for="<?=$this->get_field_id('all_m_outlcolor'); ?>">
					Outline
					<br>
					<span class="hash">#</span>
					<div class="siwraper">
		...
		<br>
		<label style="margin: 0 15px 0 0;" for="<?=$this->get_field_id('all_cf_name'); ?>" title="<span class='green'>none</span> - no Center Function;<br><span class='green'>image_cf()</span> - for an image in cloud's center;<br><span class='green'>text_cf()</span> - for text in cloud's center and<br><span class='green'>my_cf()</span> - for your own code.">
			Function
			<br>
		...
		<div style="float: left;" title="Rotation of Center <span class='green'>Function</span> image/text. Suitable for <span class='green'>square</span> sized image/text.<br><span class='green'>off</span> - no rotation;<br><span class='green'>&#8635;</span> - clockwise rotation (<span class='green'>image_cf()</span> & <span class='green'>text_cf()</span>);<br><span class='green'>&#8634;</span> - counterclockwise rotation (<span class='green'>image_cf()</span> & <span class='green'>text_cf()</span>).">
			...
			<input style="margin: 0 0 0 3px; " class="radio" id="<?=$this->get_field_id('all_cf_rotation'); ?>"
			name="<?=$this->get_field_name('all_cf_rotation'); ?>" type="radio" value="1"
			<?php if( $all_cf_rotation == "1" ){ echo ' checked="checked"'; } ?>><span style="line-height: 14px; font-size:14px; font-weight: normal;">&#8635;</span>
			
			<input style="margin: 0 0 0 3px;" class="radio" id="<?=$this->get_field_id('all_cf_rotation'); ?>"
			name="<?=$this->get_field_name('all_all_cf_rotation'); ?>" type="radio" value="-1"
			<?php if( $all_cf_rotation == "-1" ){ echo ' checked="checked"'; } ?>><span style="line-height: 14px; font-size:14px; font-weight: normal;">&#8634;</span>
		</div>
		<label style="margin-left: 15px;" title="Opacity of Center <span class='green'>Function</span> image/text" for="<?=$this->get_field_id('all_cf_opacity'); ?>">
			Opacity
			<br>
		...
		<div style="clear: both;">
			...
			<div style="display: inline-block;">
				...
				<label style="width: 69px; height: 55px;" for="<?=$this->get_field_id('all_bg_color_cf'); ?>">
					Background
					<br>
					<span class="hash">#</span>
					<div class="siwraper">
						<input title="Background color of the text: Empty for no background."
						class="colors" id="<?=$this->get_field_id('all_bg_color_cf'); ?>"
						name="<?=$this->get_field_name('all_bg_color_cf'); ?>" type="text"
						value="<?php echo $all_bg_color_cf; ?>" onblur="hex_val_check(this, this.value)" />
						<br>
						<?php 
							if($all_bg_color_cf != '') {echo '<span class="color" style="color: #' . $all_bg_color_cf . ';">&#9608;&#9608;&#9608;&#9608;&#9608;&#9608;&#9608;&#9608;&#9608;&#9608;</span>';}
						?>
					</div>
				</label>
...
?>
	...
	<h3>MENU CLOUD <span style="font-size:11px;">(menu, portfolio, slider, accordions etc.)</span></h3>
